...
export async function GET(req: NextRequest) {
  ...
  if (error || !code) {
    return NextResponse.redirect(new URL("/dashboard?error=outlook_denied", APP));
  }
  ...
  await query(
    `INSERT INTO email_connections (user_id, provider, email, access_token, refresh_token)
     VALUES (?, 'outlook', ?, ?, ?)
     ON DUPLICATE KEY UPDATE provider='outlook', email=VALUES(email),
  ...
  return NextResponse.redirect(new URL("/dashboard?connected=outlook", APP));
}
